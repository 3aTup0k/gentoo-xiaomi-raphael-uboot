...
if [ -z "$GENTOO_STAGE3_URL" ]; then
    ...
    LATEST_TXT=$(curl -sL "$GENTOO_MIRROR/releases/arm64/autobuilds/latest-stage3-arm64-openrc.txt")
    STAGE3_FILE=$(echo "$LATEST_TXT" | grep -v '^#' | head -1 | awk '{print $1}')
...
curl -sL -o "$STAGE3_FILENAME" "$GENTOO_STAGE3_URL"
