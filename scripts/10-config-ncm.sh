...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] [10] USB NCM"
...
cat > rootdir/usr/local/sbin/setup-usb-ncm.sh << 'SCRIPTEOF'
#!/bin/sh
set -e
...
cat > rootdir/etc/init.d/usb-ncm << 'EOF'
#!/sbin/openrc-run
...
start() {
    ebegin "Starting USB NCM gadget"
    /usr/local/sbin/setup-usb-ncm.sh
    eend $?
}
stop() {
    ebegin "Stopping USB NCM gadget"
    UDC=$(ls /sys/class/udc | head -n 1)
    [ -n "$UDC" ] && echo "" > /sys/kernel/config/usb_gadget/g1/UDC 2>/dev/null || true
    ip link set usb0 down 2>/dev/null || true
    eend $?
}
EOF
chmod +x rootdir/etc/init.d/usb-ncm
chroot rootdir rc-update add dnsmasq default
chroot rootdir rc-update add usb-ncm default

echo "[$(date +'%Y-%m-%d %H:%M:%S')] [10] OK"
